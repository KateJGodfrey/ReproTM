...
settings=settings_comparematrices;%what is this? Where is it coming from?
disp('settings:')
disp(settings)
...
% Verify that subs and B correspond by checking the subject identifier
good_sub = {};
good_B = {};
sub_idx = 1;
...
for i = 1:length(subs)
    ...
    subs_parts = strsplit(subs{i}, '/');
    subs_idx = find(strcmp(subs_parts, 'func'), 1);
    if ~isempty(subs_idx) && subs_idx > 1
        subs_id = strjoin(subs_parts(1:subs_idx-1), '/');
    else
        subs_id = subs_parts{1};
    end
    
    % Search for matching subject in motion files
    for j = 1:length(B)
        B_parts = strsplit(B{j}, '/');
        B_idx = find(strcmp(B_parts, 'func'), 1);
        if ~isempty(B_idx) && B_idx > 1
            B_id = strjoin(B_parts(1:B_idx-1), '/');
        else
            B_id = B_parts{1};
        end

        % disp('Check if IDs match and motion file exists')
        % disp(['subs_id: ' subs_id])
        % disp(['B_id: ' B_id])
        
        % Check if IDs match and motion file exists
        if strcmp(subs_id, B_id) && (isnumeric(B{j}) || exist(B{j}, 'file') == 2)
            disp('IDs match and motion file exists, adding to good_sub and good_B');
            good_sub{sub_idx} = subs{i};
            good_B{sub_idx} = B{j};
            sub_idx = sub_idx + 1;
            break;
        end
...
disp([num2str(length(good_sub)) ' of ' num2str(length(subs)) ' motion files found. Motion files exist continuing ...'])
disp([num2str(length(good_B)) ' of ' num2str(length(B)) ' motion files found. Motion files exist continuing ...'])
...
if length(subs)==length(B)
    ...
    error("Length of motion conc file (%d) does not match length of series conc file (%d). Check your inputs.", length(B), length(sub));
end
...
if check_motion_first ==1
    ...
    for i = 1:length(B)
        [all_subjects_minutes, all_mean_FD] = check_twins_motion(B{i}, TR, project_dir, output_name, FD_column, [], 0, 1, 0, 0);
        disp(['Subject ' num2str(i) ' has ' num2str(all_subjects_minutes) ' minutes of data after motion censoring at FD ' num2str(FD_threshold)]);
        
        if minutes_to_use <= all_subjects_minutes
            good_subs_idx = [good_subs_idx, i];
        else
            bad_subs_idx = [bad_subs_idx, i];
        end
        
        if all_subjects_minutes < 0.5
            really_bad_subs_idx = [really_bad_subs_idx, i];
        end
    end
    disp([num2str(length(good_subs_idx)) ' subjects have at least ' num2str(minutes_to_use) ' minutes of data after motion censoring at FD ' num2str(FD_threshold)]);
    disp([num2str(length(bad_subs_idx)) ' subjects have less than ' num2str(minutes_to_use) ' minutes of data after motion censoring at FD ' num2str(FD_threshold)]);
    disp([num2str(length(really_bad_subs_idx)) ' subjects have less than 30 seconds of data after motion censoring at FD ' num2str(FD_threshold)]);

else
    good_subs_idx = 1:length(subs);
end
...
if exist([project_dir filesep 'seedmaps_' file_root_no_ext '.mat'],'file') == 2
    ...
    for i=1:length(subs)
        ...
        if power_motion == 1
            %use power method
            disp(['Loading motion data from power method' B{i}]);
            load(B{i})
            ...
            if combined_outliermask_provided ==0
                if exist('remove_outliers','var') == 0 || remove_outliers == 1
                    ...
                    addpath('/projects/standard/faird/shared/code/internal/utilities/outlier-censoring/')
...
%save all maps
num_cleaned_subs = size(cleansubs, 2);
if Zscore_regions == 1
    save([project_dir filesep 'seedmaps_' file_root_no_ext '_n_' num2str(num_cleaned_subs) '_all_networksZscored.mat'],'B','seed_matrix','subs','Zscore_regions','power_motion','remove_outliers','surface_only','use_only_subjects_that_pass_motion_criteria','combined_outliermask_provided','include_scan_net','bad_subs_idx','good_subs_idx','cleansubs','FD_threshold','minutes_to_use');
else
    save([project_dir filesep 'seedmaps_' file_root_no_ext '_n_' num2str(num_cleaned_subs) '_all_networks.mat'],'B','seed_matrix','subs','Zscore_regions','power_motion','remove_outliers','surface_only','use_only_subjects_that_pass_motion_criteria','combined_outliermask_provided','include_scan_net','bad_subs_idx','good_subs_idx','cleansubs','FD_threshold','minutes_to_use');
end
