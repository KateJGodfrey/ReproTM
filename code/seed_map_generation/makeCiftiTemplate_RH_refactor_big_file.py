...
class CIFTITemplateCreator:
    ...
    def _load_cached_seedmaps(self, filepath: Path) -> List[np.ndarray]:
        ...
        with h5py.File(str(filepath), 'r') as f:
            seedmaps_data = f['seedmapstimeseries']
            return [seedmaps_data[i, 0][:] 
                    for i in range(seedmaps_data.shape[0])]
    ...
    def _save_seedmaps(self, filepath: Path, seedmaps: List[np.ndarray], 
        ...
        #     f.create_dataset('seedmapstimeseries', data=seedmaps_cell)
        #     f.create_dataset('allmasks_outliers_removed_FD02', data=masks_after_cell)
        #     f.create_dataset('allmasks_before_outliers_removed_FD02', data=masks_before_cell)
