...
# v6.0.
# major updates:
# i.   removed refinement steps, which are unfunctional, for:
#           a. dmn_greys: DMN, PMN, PON networks
#           b. sal_greys: SAL, CO networks
#      legacy code to refine these networks: */dev/template_matching_v5.0.py
# ii.  thresholding the template is now a user-specified shell script option
# minor updates:
# i.   program will terminate with sys.exit() if:
#           a. required input files (dconn, template) don't exist 
#           b. input dconn and input template differ in number of greyordinates
# ii.  removed warnings about refinement step as refinement is functioning correctly
...
template_thresholding=[sys.argv[3]][0].lower() in ('yes','y')
template_minthreshold=[sys.argv[4]][0]
# refinement options
refine_template_matching=[sys.argv[5]][0].lower() in ('yes','y')
refine_minimum_threshold=[sys.argv[6]][0]
# output matlab file
matfile_out = [sys.argv[7]][0]
# output dscalar files
dscalar_template_infile = [sys.argv[8]][0]
dscalar_outfile = [sys.argv[9]][0]
#print(dscalar_outfile)
dscalar_outfile_refined = [sys.argv[10]][0]
...
if refine_template_matching:
    REFINEMENTTEMPLATEMINIMUM = float(refine_minimum_threshold)
network_names = ['DMN','Vis','FP','','DAN','','VAN','Sal','AMN','SMd','SMl','Aud','Tpole','MTL','PMN','PON','','SCAN'] # changed CO to AMN (8th label) - Tanya Pandhi 03-10-2026
...
if save_first_step_mat:
    ...
    # matfile_out='/projects/standard/midb_abcd/shared/ABCC/code/precision_mapping_via_template_matching/compare_matrices_python/code/template_matching_python/support_files/sub-5T6MEVTN_ses-combined_task-rest_noFisher_zscoreRH2.mat'    
...
if load_old_mat:
    ...
    mat_filepath='/projects/standard/midb_abcd/shared/ABCC/code/precision_mapping_via_template_matching/compare_matrices_python/code/template_matching_python/support_files/sub-5T6MEVTN_ses-combined_task-rest_TM.mat'
...
if refine_template_matching:
    print('template matching refinement has begun')
    print('')
    ...
    if refine_motor_grays:
        ...
        print('\trefinement of motor and scan grays:')
        steptimer=time.time()
        ...
        for i in range(motor_grays.shape[0]):
            ...
            if i % 5000 == 0:
                print('\t\tmotor and scan grey refinement has run for: ' + str(round((time.time()-steptimer)/60,3)) + ' minutes')
        ...
        print('\t\trefinement of motor and scan grays complete')
        print('\t\tindividual step took: ' + str(round((time.time()-steptimer)/60,3)) + ' minutes')
        print('\t\t')
