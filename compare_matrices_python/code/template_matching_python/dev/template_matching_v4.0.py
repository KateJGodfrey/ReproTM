...
# test that input cifti and template are same units?
# does this work without wb_command?
# TO DO
# rather than hardcoding networks to skip, skip networks if name is blank
# note that subject labels array is being saved in transpose - might be nice to change
# include a debug mode option to print first few elements of input and template matrix
...
# v4.0 
# major updates:
# i.  returned to returning NaN if cmap == 0 or tmap == 0 as this matches matlab src code functionality
#     confirmed matching functionality by modifying matlab src code to save both eta matrices
...
#     now saving as same datatype in output .mat file
...
# input files
dconn_infile = '/sub-10227/ses-combined/func/sub-10227_ses-combined_task-crossMENORDICtrimmed_space-fsLR_den-91k_desc-denoised_bold_spatially_interpolated_smoothed2mm_censoredFD2mm_fisherZ_zscoreRH.dconn.nii'
...
# output files
matfile_out='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/template_matching/v6_interpolated/sub-10227_ses-combined_task-crossMENORDICtrimmed_space-fsLR_den-91k_desc-denoised_bold_spatially_interpolated_smoothed2mm_censoredFD2mm_fisherZ_zscoreRH_pythonTM.mat'
...
file_checker=True  # can probably be moved to shell script
if file_checker:
    # check for input dconn
    if os.path.isfile(dir_in + dconn_infile) == False:
...
if refine_scan_network:
    ...
    new_subject_labels_r_scan=new_subject_labels_r.copy()
    ...
    print('\ttemplate matching for scan network complete')
...
# restore_old = False
# if restore_old:
#     # new_subject_labels_copy = new_subject_labels.copy()
#     # new_subject_labels_scan_copy = new_subject_labels_scan.copy()
#     # new_subject_labels_r_copy = new_subject_labels_r.copy()
#     # new_subject_labels_r_scan_copy = new_subject_labels_r_scan.copy()
    
#     new_subject_labels = new_subject_labels_copy.copy()
#     new_subject_labels_scan = new_subject_labels_scan_copy.copy()
#     new_subject_labels_r = new_subject_labels_r_copy.copy()
#     new_subject_labels_r_scan = new_subject_labels_r_scan_copy.copy()

#matfile_out='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/template_matchingv3.0/sub-10227_ses-combined_task-cross_noFisher_TM4.mat'
#dscalar_outfile='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/template_matchingv3.0/sub-10227_ses-combined_task-crossMENORDICtrimmed_smoothed2mm_censoredFD2mm_noFisher_zscoredRH_TM.dscalar.nii'
...
if save_outputs:
    ...
    if refine_scan_network:
        ...
        new_subject_labels_scan = np.array([x + 1 for x in new_subject_labels_scan])
        new_subject_labels_r = np.array([x + 1 for x in new_subject_labels_r])
        new_subject_labels_r_scan = np.array([x + 1 for x in new_subject_labels_r_scan])
        
        mat_dict={"network_names": np.array(network_names,dtype=object)
                ,"eta_to_template_vox": eta_to_template_vox
                ,"eta_to_template_vox_modified": eta_to_template_vox_modified
                ,"new_subject_labels": np.array(new_subject_labels,dtype=int)
    ...
    # test = new_subject_labels_scan.copy()
    # test2 = test.reshape(1,91282)
    
    # dscalar_template = nb.load(dscalar_template_infile)
    # new_img = nb.Cifti2Image(test2, header=dscalar_template.header,
    #                         nifti_header=dscalar_template.nifti_header)
    # new_img.to_filename(dscalar_outfile)

    # use a previous dscalar as a template
    # dscalar_template_infile='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/sub-10227_ses-combined_task-cross_grims_recoloredCifti2.dscalar.nii'
    # dscalar_outfile='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/template_matchingv2.0_Z/sub-10227_ses-combined_task-cross_dscalar5.dscalar.nii'

    # dscalar_template = nb.load(dscalar_template_infile)
    # dscalar_dat = dscalar_template.get_fdata()
    # labels = dscalar_template.dataobj
    # surface_dat=cifti2.Cifti2Surface

    # new_img = nb.Cifti2Image(new_subject_labels
    #                         ,header=dscalar_template.header
    #                         ,nifti_header=dscalar_template.nifti_header
    #                         ,extra=scalar_axis)
    # new_img.to_filename(dscalar_outfile)

    #main_dir='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching'

    #dscalar_template_infile = main_dir + '/support_files/sub-10227_ses-combined_task-cross_grims_recoloredCifti2.dscalar.nii'
    #dscalar_outfile= main_dir + '/derivatives/template_matchingv3.0/sub-10227_ses-combined_task-cross_noScan.dscalar.nii'
    ...
    new_img = nb.Cifti2Image(new_subject_labels_scan.reshape(1,greyordinates)
                            ,header=dscalar_template.header
                            ,nifti_header=dscalar_template.nifti_header
                            ,extra=scalar_axis)
    new_img.to_filename(dscalar_outfile)
