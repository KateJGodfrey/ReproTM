...
% update log
% same at working v1 script which can produce .mat file
% but added sections throughout to print info about variables

% module load matlab/R2019a

dir_in='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/dconn';
cifti_output_folder='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/derivatives/template_matching/v3_fisher/matlab';
dconn_file='/sub-10227/ses-combined/func/sub-10227_ses-combined_task-crossMENORDICtrimmed_space-fsLR_den-91k_desc-denoised_bold_smoothed2mm_censoredFD2mm_zscoreRH.dconn.nii';
template_path='/home/btervocl/shared/projects/MINT/kgodfrey/template_matching/seedmaps_subs_withsmoothed_dtseries_n141_all_networksZscored.mat';
output_cifti_name='sub-10227_ses-combined_task-crossMENORDICtrimmed_space-fsLR_den-91k_desc-denoised_bold_smoothed2mm_censoredFD2mm_zscoreRH_matlabTM';
TEMPLATEMINIMUM = 1;
...
for i=1:size(corr_mat_full,1)
    ...
    cmap = corr_mat_full(i,goodvox)';           % get info on vector in terminal: sum(cmap) or cmap(1:5)
    tmap = cifti_template_mat_full(goodvox,j);  % get info on vector in terminal: sum(tmap) or tmap(1:5)

    % Mgrand is a single numeric value
    % calculated by summing all values of cmap and tmap divided by number of values
    Mgrand  = (mean(mean(tmap)) + mean(mean(cmap)))/2;

    % Mwithin is done element wise, to get the mean of each cmap tmap unit pair
    Mwithin = (tmap+cmap)/2;
