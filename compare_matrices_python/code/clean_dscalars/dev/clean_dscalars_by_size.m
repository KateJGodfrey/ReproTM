...
% /projects/standard/faird/shared/code/internal/analytics/compare_matrices_to_assign_networks
...
code_dir='/projects/standard/faird/shared/code/internal/analytics/compare_matrices_to_assign_networks';
support_folder='/projects/standard/faird/shared/code/internal/analytics/compare_matrices_to_assign_networks/support_files';
disp(strcat('code_dir: ',code_dir));
disp(strcat('support_folder: ',support_folder));
disp('');
...
if isdeployed
    ...
    wb_command='/projects/standard/faird/shared/code/external/utilities/workbench/1.5.0/workbench/bin_linux64/wb_command'; %path to wb_command
    warning('on')
end
...
for i=1:length(dscalarwithassignments)
    ...
    if make_consensus == 1
        ...
        %edit JM: add potential colors for Alyssa's template
        %potential_colors = [1 2 10 9 3 5 11 16 15 7 8 12 14 13];%[1 2 10 9 3 5 6 11 16 15 7 8 17 12 4 14 13];
        potential_colors = [1 4 10 13 8 12 5 3 9 6 2 7 11 16 15];
        ...
        % 
        %         end
        %update: use values from Alyssa's network solution
        if remove46 == 1 % set values that are either 4 or 6 to zero (for missing networks).
            all_recolored(all_recolored==3) = 0;
            all_recolored(all_recolored==7) = 0;
            all_recolored(all_recolored==9) = 0;
            all_recolored(all_recolored==12) = 0;
            all_recolored(all_recolored==14) = 0;
            all_recolored(all_recolored==15) = 0;
            all_recolored(all_recolored==16) = 0;
    ...
    dotsloc = strfind(regularized_ciftifile,'.');
    basename = regularized_ciftifile(1:(dotsloc(end-1)-1));
    outname = [ basename '_recolored' '_minsize' num2str(minsize) ]; 
